...
-- Thu Jun 22 22:35:35 2017
-- Model: sena_ERD    Version: sena-core-1.3.0
...
CREATE INDEX `i_stock_prices3` ON `sena_db`.`stock_prices` (`split_flg` ASC, `date` ASC, `stock_code` ASC);

CREATE INDEX `i_stock_prices4` ON `sena_db`.`stock_prices` (`split_flg` ASC, `stock_code` ASC, `date` ASC);


-- -----------------------------------------------------
-- Table `sena_db`.`rule_templates`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `sena_db`.`rule_templates` ;

CREATE TABLE IF NOT EXISTS `sena_db`.`rule_templates` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '株価推移ルールテンプレートID',
  `title` VARCHAR(64) NOT NULL COMMENT 'タイトル',
  `type` INT NOT NULL COMMENT '種別	（-1：下降、0：停滞、1：上昇）',
  `price_template` TEXT NOT NULL COMMENT '株価テンプレート	（json配列の株価推移を表すテンプレート：{\"stock_prices\":[1.1,1.2,1.3,1.4]}）',
  `created` DATETIME NOT NULL COMMENT '作成日時',
  `modified` DATETIME NOT NULL COMMENT '更新日時',
  `delete_flg` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '削除フラグ	（1：削除、0：未削除）',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
COMMENT = '株価推移ルールテンプレート';

CREATE INDEX `i_rule_templates1` ON `sena_db`.`rule_templates` (`delete_flg` ASC, `type` ASC);

CREATE INDEX `i_rule_templates2` ON `sena_db`.`rule_templates` (`type` ASC, `delete_flg` ASC);


-- -----------------------------------------------------
-- Table `sena_db`.`price_statuses`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `sena_db`.`price_statuses` ;

CREATE TABLE IF NOT EXISTS `sena_db`.`price_statuses` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '株価ステータスID',
  `date` DATE NOT NULL COMMENT '日付',
  `stock_code` INT NOT NULL COMMENT '銘柄コード',
  `rule_templates_id` BIGINT NOT NULL,
  `created` DATETIME NOT NULL COMMENT '作成日時',
  `modified` DATETIME NOT NULL COMMENT '更新日時',
  `delete_flg` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '削除フラグ	（1：削除、0：未削除）',
  PRIMARY KEY (`id`))
...
CREATE INDEX `fk_price_statuses_rule_templates1_idx` ON `sena_db`.`price_statuses` (`rule_templates_id` ASC);

CREATE INDEX `fk_price_statuses_stocks1_idx` ON `sena_db`.`price_statuses` (`stock_code` ASC);

CREATE INDEX `i_price_statuses1` ON `sena_db`.`price_statuses` (`stock_code` ASC, `date` ASC);

CREATE INDEX `i_price_statuses2` ON `sena_db`.`price_statuses` (`date` ASC, `stock_code` ASC);

CREATE INDEX `i_price_statuses3` ON `sena_db`.`price_statuses` (`rule_templates_id` ASC, `date` ASC);

CREATE INDEX `i_price_statuses4` ON `sena_db`.`price_statuses` (`rule_templates_id` ASC, `stock_code` ASC);

CREATE INDEX `i_price_statuses5` ON `sena_db`.`price_statuses` (`stock_code` ASC, `rule_templates_id` ASC);
